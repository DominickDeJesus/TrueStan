...
const PlayPage = ({
	...
	/**Randomizes the order of the array and returns a new one.
	 * @returns a new rondomly sorted array
	 * @param {*} array
	 */
	...
	const isGuessCorrect = (usrGuess, correctAns) => {
		...
		if (answer === guess) {
			...
			setRound(1);
			const shuffledArr = randomizeArray(artistObj.results);
			setCurrentTrack(shuffledArr[0]);
			setPickedSongs([shuffledArr[0].trackName]);
			setCurrentTrack(shuffledArr[0]);
